...
void Link_To_Touchplate(unsigned short touchlocx, unsigned short touchlocy,
				   ...
				   int type)
{
	...
	MAPSPOT(tilex, tiley, 0) =
		(unsigned short)(lastpwallobj->areanumber + AREATILE);
	...
	if ((pw->tilex != checkx) || (pw->tiley != checky))
	{
		...
		if ((area <= 0) || (area > NUMAREAS))
		{
			...
			MAPSPOT(pw->tilex, pw->tiley, 0) =
				(unsigned short)(pw->areanumber + AREATILE);
		}
