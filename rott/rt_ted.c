...
//
#if (SHAREWARE == 1)
		CA_RLEWexpand((unsigned short *)buffer,
					  (unsigned short *)mapplanes[plane], expanded >> 1,
					  SHAREWARE_TAG);
#else
		CA_RLEWexpand((unsigned short *)buffer,
					  (unsigned short *)mapplanes[plane], expanded >> 1,
...

{
	...
	for (plane = 0; plane <= 2; plane++)
	{
		...
		// unRLEW, skipping expanded length
		//
		CA_RLEWexpand((unsigned short *)(buffer + 2),
					  (unsigned short *)mapplanes[plane], expanded >> 1,
					  0xabcd);
			...
			switch (tile)
			{
				...
				case 79:
					if (ActorIsPushWall(i, j))
					{
						if (MAPSPOT(i, j, 2))
						{
							touchx = (unsigned short)((MAPSPOT(i, j, 2) >> 8) &
													  0xff);
							touchy = (unsigned short)((MAPSPOT(i, j, 2) >> 0) &
													  0xff);
		...
		for (i = 0; i < mapwidth; i++)
		{
			if (MAPSPOT(i, j, 2))
			{
				if (IsDoor(i, j) == 1)
				{
					...
					{
						touchx =
							(unsigned short)((MAPSPOT(i, j, 2) >> 8) & 0xff);
						touchy =
							(unsigned short)((MAPSPOT(i, j, 2) >> 0) & 0xff);
...
void SetupClocks(void)
{
	...
	unsigned short *map, tile, mapx, mapy;
