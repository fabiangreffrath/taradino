...
static bool8_t MissileSound      =    TRUE;




bool8_t FirstExplosionState(statetype *state)
   {
...
void SpawnNewObj (uint32_t tilex, uint32_t tiley, statetype *state, classtype which)
   {
   int newarea;
...
void SpawnDisk(int tilex, int tiley, int type, bool8_t master)
{int zoffset;
 ...
 if (master == TRUE)
   ...
   if (!type)
      ...
      if ((zoffset&0xff00)==0xb000)
         Set_NewZ_to_MapValue((int32_t*)(&(new->temp2)),zoffset,"elev disk",tilex,tiley);
      else
         new->temp2 = 32;
      new->temp1 = 1;
      }
      ...
      zoffset = MAPSPOT(tilex,tiley,2);
      if ((zoffset&0xff00)==0xb000)
         Set_NewZ_to_MapValue((int32_t*)(&(new->z)),zoffset,"path disk",tilex,tiley);
...
bool8_t Vicious_Annihilation(objtype *ob, objtype *attacker)
   {
...
bool8_t MissileTryMove(objtype*ob,int tryx,int tryy,int tryz)
{
...
bool8_t EluderCaught(objtype*ob)
{
 objtype *temp;
 int dx,dy,dz;
 int dist = 0xc000;
...
bool8_t CheckDoor(objtype *ob,doorobj_t * door,int trytilex,int trytiley)
{bool8_t doorok=FALSE;
...
bool8_t WallCheck(int tryx,int tryy)
...
bool8_t QuickSpaceCheck(objtype*ob,int tryx, int tryy)
...
movement_status CheckOtherActors(objtype*ob,int tryx,int tryy,int tryz)
   ...
   bool8_t bouncer,pusher,thinkingactor,zstoppable,ACTORSTOP;
...
movement_status CheckStaticObjects(objtype *ob,int tryx,int tryy,int tryz)
  ...
  bool8_t specialstat=FALSE,widestat=FALSE,zstoppable;
...
bool8_t ActorTryMove(objtype*ob,int tryx, int tryy, int tryz)
   {
...
findplayer:
   if (ob->temp1 == -1)
      ...
      if ((head->dir == (uint32_t)tdir) && (ob->dir == (uint32_t)tdir)) // increment
...
bool8_t NMEspincheck(objtype*ob)
   {
   int dx,dy,dz;
...
void T_NME_SpinFire(objtype*ob)
{
 ...
 if (ob->dir != (uint32_t)ob->targettilex)
...
void SelectSnakeDir (objtype *ob)
{
 ...
 if ((spot >= 0) && (spot<= 7) && ((ob->dir!=(uint32_t)spot)||(!(ob->flags & FL_DONE))))
...
void T_DarkSnakeChase(objtype*ob)
   ...
   angle = AngleBetween(ob,PLAYER[0]);
   tdir = angletodir[angle];
   if (Near(ob,PLAYER[0],6) && (ob->dir == (uint32_t)tdir) && (!(ob->state->condition & SF_DOWN)))
      {
...
//
	uint32_t absdx = abs(dx);
	uint32_t absdy = abs(dy);
...
bool8_t CheckSight (objtype *ob,void *atwhat)
{
	long     deltax,deltay;
	objtype * what;
//
...
bool8_t SightPlayer (objtype *ob)
   {
	//if (ob->flags & FL_ATTACKMODE)
...
bool8_t CheckLine (void *from, void *to, int condition)
{
