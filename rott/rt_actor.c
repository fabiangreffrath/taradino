...
void SpawnNewObj(unsigned int tilex, unsigned int tiley, statetype *state,
				 classtype which)
{
	int newarea;
...
findplayer:
	...
	else
	{
		...
		if ((head->dir == (unsigned int)tdir) &&
			(ob->dir == (unsigned int)tdir)) // increment
...
void SelectSnakeDir(objtype *ob)
{
	...
	if ((spot >= 0) && (spot <= 7) &&
		((ob->dir != (unsigned int)spot) || (!(ob->flags & FL_DONE))))
	{
...
void T_DarkSnakeChase(objtype *ob)
{
	...
	angle = AngleBetween(ob, PLAYER[0]);
	tdir = angletodir[angle];
	if (Near(ob, PLAYER[0], 6) && (ob->dir == (unsigned int)tdir) &&
		(!(ob->state->condition & SF_DOWN)))
	{
